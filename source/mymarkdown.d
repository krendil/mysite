...
*/
string filterMyMarkdown()(string str, MyMarkdownFlags flags = MyMarkdownFlags.vanillaMarkdown)
{
	auto dst = appender!string();
	filterMyMarkdown(dst, str, flags);
	return dst.data;
}
...
*/
void filterMyMarkdown(R)(ref R dst, string src, MyMarkdownFlags flags = MyMarkdownFlags.vanillaMarkdown)
{
...
enum MyMarkdownFlags {
	none = 0,
	keepLineBreaks = 1<<0,
	noInlineHtml = 1<<2,
	vanillaMarkdown = none,
	forumDefault = keepLineBreaks|noInlineHtml
}
...
private Line[] parseLines(ref string[] lines, MyMarkdownFlags flags)
{
	Line[] ret;
	while( !lines.empty ){
		auto ln = lines.front;
		...
		if( isCodeBlockDelimiter(ln) ) lninfo.type = LineType.CodeBlockDelimiter;
		...
		else if( !(flags & MyMarkdownFlags.noInlineHtml) && isHtmlBlockLine(ln) ) lninfo.type = LineType.HtmlBlock;
...
private void parseBlocks(ref Block root, ref Line[] lines, IndentType[] base_indent, MyMarkdownFlags flags)
{
...
/// private
private void writeBlock(R)(ref R dst, ref const Block block, LinkRef[string] links, MyMarkdownFlags flags)
{
	final switch(block.type){
		...
		case BlockType.Text:
			foreach( ln; block.text ){
				...
				if( flags & MyMarkdownFlags.keepLineBreaks ) dst.put("<br>");
				dst.put("\n");
			}
			foreach(b; block.blocks)
				writeBlock(dst, b, links, flags);
		...
		case BlockType.Code:
			assert(block.blocks.length == 0);
            auto lang = "";
            if( auto langp = "lang" in block.info ) {
                lang = *langp;
            }
...
/// private
private void writeMarkdownEscaped(R)(ref R dst, string ln, in LinkRef[string] linkrefs, MyMarkdownFlags flags)
{
	bool br = ln.endsWith("  ");
	while( ln.length > 0 ){
		switch( ln[0] ){
			...
			case '!':
				...
				break;
			case '>':
				if( flags & MyMarkdownFlags.noInlineHtml ) dst.put("&gt;");
				else dst.put(ln[0]);
				ln = ln[1 .. $];
				break;
			case '<':
...
{
    // check CTFE-ability
    enum res = filterMyMarkdown("### some markdown\n[foo][]\n[foo]: /bar");
